...
char sifre[4] = { '*', '#', 'C', '9' }; //default password we can change it from here and also change it int he void loop!
...
void setup() {
    ...
    lcd.clear();
    lcd.setCursor(1, 0);
    lcd.print("SMART HOME");//Clear screen
    ...
    lcd.setCursor(0, 1);
    lcd.print("HIRSIZ ALARM");   // LOCK DOOR SYSTEM
    delay(2000);
    lcd.clear();
    lcd.setCursor(0, 0);
...
void loop() {
    ...
    {
        delay(200);
        char sifre[4] = { '*', '#', 'C', '9' }; //default password u should write here like that 
